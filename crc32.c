 ...
 *
 *  This crc32 implementaion is based on public domain code
 *
...
UHID_NO_EXPORT int CRC32FromFile(const char *path, uint32_t *outCrc32)
{
    int ret;
    FILE *fd = fopen(path, "rb");
    if (!fd)
        return -1;
    ret = CRC32FromFd(fd, outCrc32);
    return ret;
}
...
UHID_NO_EXPORT int CRC32FromFd( FILE *file, uint32_t *outCrc32 )
{
    ...
    *outCrc32 = 0;
    while (1) {
        ...
        *outCrc32 = CRC32FromBuf( *outCrc32, buf, bufLen );
    }
    ret = 0;
